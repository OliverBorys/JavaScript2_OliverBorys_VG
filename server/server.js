...
app.get("/api/admin/users", (req, res) => {
  res.json(adminDb.prepare("SELECT id, username FROM admin_users").all()); // Hide passwords
});
...
app.post("/api/update-hero-images", (req, res) => {
  ...
  if (!imageUrl1 || !imageUrl2) return res.status(400).json({ error: "Both image URLs are required." });

  heroDb.prepare("UPDATE hero_images SET image_url = ? WHERE id = 1").run(imageUrl1);
  heroDb.prepare("UPDATE hero_images SET image_url = ? WHERE id = 2").run(imageUrl2);
